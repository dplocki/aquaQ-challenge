...
def conversion_process(value: str) -> str:
    ...
    characters = list(
        map(lambda character: character if is_hex(character) else "0", extended_value)
    )
    characters_size = require_size // 3

    return "".join(
        characters[:characters_size][:2]
        + characters[characters_size : characters_size * 2][:2]
        + characters[characters_size * 2 :][:2]
    )
