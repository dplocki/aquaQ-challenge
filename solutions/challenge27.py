...
def walk(was: Set[Tuple[int, int]], _map: Dict[Tuple[int, int], str], start_point: Tuple[int, int]) -> Generator[str, None, None]:
    ...
    while True:
        ...
        new_current_point = current_point[0] + direction[0], current_point[1] + direction[1]

        if new_current_point not in _map:
            neighbors = [n for n in only_valid_neighbors(_map, current_point) if n not in was]
            if len(neighbors) == 0:
                yield result
                return
...
def solution(content: str) -> int:
    ...
    result = 0
    was = set()
    for start in sorted(find_snake_endings(_map), key=lambda coordinates: coordinates[1]):
        for word in walk(was, _map, start):
            result += sum(ord(letter) - letter_a + 1 for letter in word) * len(word)
